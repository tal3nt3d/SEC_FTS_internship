...
class CommentService:
    ...
    async def service_create_comment(self, task_id: int, comment_data: CommentCreate):
        ...
        self.comments_db.append(comment_dict)
        return CommentResponse(**comment_dict)
    
    async def service_get_comments(self, task_id: int):
        task_exists = any(t["id"] == task_id for t in self.tasks_db)
        if not task_exists:
            raise TaskNotFoundError()
